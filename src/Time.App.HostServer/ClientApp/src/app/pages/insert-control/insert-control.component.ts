...
import { InsertControlService } from 'src/app/services/insert-control.service';
import { ActiveLinkENUM } from 'src/app/common/enum/sidebar.enum';
import { IdName, IdNameLevel, InsertControl } from 'src/app/models/general.models';
...
@Component({
  selector: 'app-insert-control',
  templateUrl: './insert-control.component.html',
  styleUrls: ['./insert-control.component.css']
})
export class InsertControlComponent implements OnInit, AfterContentChecked, AfterViewChecked {
  /* 
  ...
  */
  ...
  constructor(
    private insertControlService: InsertControlService,
  ...
  getAgendaTypeList(): void {
    this.calendarService.getAgendaTypeList(1,1000).subscribe(result => {
      //this.agendaTypeList = result
    })
  ...
  updateRange(event: string = ''): void {
    ...
    this.insertControlService.getInsertControlList(param).subscribe((result: InsertControl[]) => {
      if (result.length > 0) {
        if (this.page > 1) {
